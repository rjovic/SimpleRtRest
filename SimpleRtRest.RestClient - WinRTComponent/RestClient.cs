...
namespace SimpleRtRest.RestClient
{
    internal class RestClient : IRestClient
    {
        ...
        public async Task<IRestResponseGeneric<T>> Execute<T>(IRestRequest request) where T : new()
        {
            return Deserialize<T>(await Execute(request), request);
        }

        private RestResponseGeneric<T> Deserialize<T>(IRestResponse raw, IRestRequest request)
        {
            if (raw.StatusCode == HttpStatusCode.OK || raw.StatusCode == HttpStatusCode.Created)
            {
                var response = new RestResponseGeneric<T>();
                try
                {
            ...
            return new RestResponseGeneric<T> { StatusCode = raw.StatusCode };
        }
    }
}
